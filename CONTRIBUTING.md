...
5. **Testing**
   Please test your changes thoroughly before submitting a pull request. Include any relevant test cases to ensure the stability of the project.

   - **TypeScript SDK**: uses [Vitest](https://vitest.dev/) — run `npm test` inside `./typescript`.
   - **Python SDK**: uses [pytest](https://pytest.org/) — run `pytest` inside `./python`.
   - **Java SDK**: uses [JUnit 5](https://junit.org/junit5/) — run `mvn test` inside `./java`.
